...
class TwbParser:
    ...
    def parse_datasources(self) -> list[dict[str, Any]]:
        ...
        for ds in candidates:
            ...
            # Attach per-table column lists (if discoverable from
            # metadata-records) onto each table dict in `relations`.
    ...
    def _build_object_table_map(self, ds_element: ET.Element) -> dict[str, str]:
        ...
        for obj in ds_element.findall(".//object-graph/objects/object"):
            ...
            if relation is not None:
                # Prefer the clean display `name` over the bracketed
                # physical `table` ref, so this matches how table nodes
                # are keyed in _parse_relations (which also prefers
                # `name`) -- otherwise relationships resolved through this
                # id map end up tagged with a bracketed table string that
                # never matches the diagram's node names.
    ...
    def _parse_table_columns(self, ds_element: ET.Element) -> dict[str, list[str]]:
        """Maps each physical table name to its list of column names, read
        from <connection><metadata-records><metadata-record class="column">
        entries. Tries several tag-name variants since Tableau's export
        shape differs slightly by connector/version. Returns {} (no crash,
        no columns rendered) if none of the expected tags are found."""
