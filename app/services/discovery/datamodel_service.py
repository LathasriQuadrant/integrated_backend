...
async def discover_data_model(
    ...
    workbook_id: str,
    twb_data: dict[str, Any] | None,
    workbook_graph: dict[str, Any] | None = None,
) -> dict[str, Any]:
    connections = await rest_client.get_workbook_connections(workbook_id)
    ...
    # The Metadata API (GraphQL) is the only source of table/column data for
    # *published* datasources -- twb_data only covers embedded ones, and the
    # REST connections loop above only records the datasource name/server,
    # never its tables. Merge the workbook's upstream tables (with columns)
    # in here: backfill columns onto twb-derived tables that matched by
    # name, and add any published-datasource tables twb_data never saw.
    graph_tables = (workbook_graph or {}).get("upstreamTables", [])
    for gt in graph_tables:
        gt_name = gt.get("name", "")
        if not gt_name:
            continue
        gt_columns = [c.get("name", "") for c in gt.get("columns", []) if c.get("name")]

        matched = False
        for t in tables:
            t_name = t.get("table") or t.get("name", "")
            if t_name.strip("[]") == gt_name.strip("[]"):
                matched = True
                if not t.get("columns"):
                    t["columns"] = gt_columns
